...
if ($_SERVER["REQUEST_METHOD"] == "GET") {
    ...
    $BDAcontrole = floatval($_GET['NT_cntrlBDA'] ?? 0);
    $BDACC = floatval($_GET['NT_CCBDA'] ?? 0);
    $MSRcontrole = floatval($_GET['NT_cntrlMSR'] ?? 0);
    $MSRCC = floatval($_GET['NT_CCMSR'] ?? 0);
    $PARcontrole = floatval($_GET['NT_cntrlPAR'] ?? 0);
    $PFEcontrole = floatval($_GET['NT_cntrlPFE'] ?? 0);

    $moyenneG2= ($BDAcontrole * 0.7 + $BDACC * 0.3) * 1 +
                ($MSRcontrole * 0.7 + $MSRCC * 0.3) * 1 +
                ($PARcontrole * 1) + 
                ($PFEcontrole * 3) ;

    $moyenneTT2 = $moyenneG2/6;

    $formatted_moyenne = number_format($moyenneTT2, 2);

    $status = $moyenneTT2 > 10 ? 'success' : 'danger';
    $status_text = $moyenneTT2 > 10 ? 'Validé' : 'Non validé';
    $message = $moyenneTT2 > 10 ? 
